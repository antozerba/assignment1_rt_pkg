...
class DistanceNode : public rclcpp::Node {
    ...
    DistanceNode() : Node("distance_node")
    {
        ...
        subscriber1_ = this->create_subscription<nav_msgs::msg::Odometry>
        ("/robot1/odom", 10, std::bind(&DistanceNode::gettin_pose1, this, std::placeholders::_1));
        subscriber2_ = this->create_subscription<nav_msgs::msg::Odometry>
        ("/robot2/odom", 10, std::bind(&DistanceNode::gettin_pose2, this, std::placeholders::_1));
        ...
        turtle1_vel_pub_ = this->create_publisher<geometry_msgs::msg::Twist>("/robot1/cmd_vel",10);
        turtle2_vel_pub_ = this->create_publisher<geometry_msgs::msg::Twist>("/robot2/cmd_vel",10);
    ...
    //
    rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr subscriber1_;
    rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr subscriber2_;
    ...
    bool boundaries(const Pose2D &pose){
        return (pose.x < bound_min || pose.x > bound_max ||
                pose.y < bound_min || pose.y > bound_max );
    }
    void gettin_pose1(const nav_msgs::msg::Odometry::SharedPtr msg){
        pose1.x = msg->pose.pose.position.x;
        pose1.y = msg->pose.pose.position.y;
    }
    void gettin_pose2(const nav_msgs::msg::Odometry::SharedPtr msg){
        pose2.x = msg->pose.pose.position.x;
        pose2.y = msg->pose.pose.position.y;
    }
